...
export default function calculateMatrix(birthDate: string) {
  const [year, month, day] = String(birthDate).split("-");
  const а = checkTwentyTwo(Number(day));
  const б = checkTwentyTwo(Number(month));
  const в = checkTwentyTwo([...year].reduce((acc, value) => acc + +value, 0));
  ...
  const healthCartThird = {
    ...
    wishudha: checkTwentyTwo(svgMatrixChartProps?.А1 + svgMatrixChartProps?.Б1),
    anahata: checkTwentyTwo(svgMatrixChartProps?.А3 + svgMatrixChartProps?.Б3),
    manipura: checkTwentyTwo(svgMatrixChartProps?.Д + svgMatrixChartProps?.Д),
    svadhistana: checkTwentyTwo(
      svgMatrixChartProps?.Г1 + svgMatrixChartProps?.В1
    ),
    muladhara: checkTwentyTwo(svgMatrixChartProps?.В + svgMatrixChartProps?.Г),
  };
